...
# @Concepto: mig_laser_diagnostic.py 
    ...
    def run_migration(self):
        ...
        while attempt <= max_retries:
            ...
            try:
                ...
                print(f"\n✅ Migración completada: {total_inserted} registros migrados de historiales de tipo diagnóstico")
        ...
        SELECT * FROM (	
					SELECT 
						...
						CONCAT(
                                ...
                                if(REVISIONEXTERNA=0,'',' - Revisión externa: Si\n'),								
								' - Tipo: ',IFNULL(TIPO,' - '),'\n',
					...
					SELECT 
						...
						0 AS LaserGTicketGID,
						COALESCE(r.OPERADORASOCIADO_KEY_OID, (SELECT `KEY` FROM OPERADORDEFRANQUICIA WHERE REPLACE(CONCAT(TRIM(NOMBRE),' ',TRIM(APELLIDOS)),'  ',' ')=REPLACE(TRIM(NOMBREOPERADOR),'  ',' ') LIMIT 1 ),0) AS LaserGUserIDFrom,
					...
					WHERE  
                        t.TIPO IN ('Calor','Aspirado') 
                        AND  IF(@@server_uuid='02d35120-2210-11e9-80f9-42010a800117',r.FECHAREVISION>'2024-12-31',r.FECHAREVISION<='2024-12-31')
                        AND (r.CENTROASOCIADO_KEY_OID=r.CENTROEXTERNOASOCIADO_KEY_OID OR r.CENTROASOCIADO_KEY_OID IS NULL)
                    ...
                    SELECT 
                        ...
                        CONCAT(		
                            ...
                            ' - Operador: ',IFNULL(r.NOMBREOPERADOR,' - '),'\n',
                            ' - Observaciones: ',IF(r.OBSERVACIONES='' OR ISNULL(r.OBSERVACIONES) ,' Sin anotaciones',r.OBSERVACIONES),'\n',					
                            ' - Anotaciones revisión externa: ',IF(t.OBSERVACIONESREVISIONEXTERNA='' OR ISNULL(t.OBSERVACIONESREVISIONEXTERNA) ,' Sin anotaciones',t.OBSERVACIONESREVISIONEXTERNA)			
                        ) AS LaserGComments,
                        CURDATE() AS LaserGStamp,
                        0 AS  __LaserGIDTto,
                        r.`KEY` AS __LaserGIDRev,
                        0 AS __LaserGIDMto,
                        24 AS __LaserGProductSourceID,
                        -1 AS __LaserMigrated,
                        r.CENTROEXTERNOASOCIADO_KEY_OID AS __LaserExtRwClinicID
                    FROM REVISION r 
                    INNER JOIN TRATAMIENTO t ON t.`KEY`=r.TRATAMIENTOASOCIADO_KEY_OID
                    WHERE  
                            t.TIPO IN ('Calor','Aspirado') 
